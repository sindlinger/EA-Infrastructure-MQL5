...
public:
    ...
    virtual TradingSignal GetSignal() {
        ...
        if(!m_distanceMgr.CheckDistance()) {
            signal.reason = "Distância insuficiente";
            if(m_debug) {
                double currentDist = m_distanceMgr.GetCurrentDistance();
                double minDist = m_distanceMgr.GetMinDistance();
                Print("❌ HedgeLine: Distância insuficiente - ",
        ...
        double distance = m_distanceMgr.GetATRDistance();
