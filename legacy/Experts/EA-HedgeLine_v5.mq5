//+------------------------------------------------------------------+
//|                                          EA-HedgeLine_v5         |
//|                    Sistema HedgeLine v5 - Versão Final           |
...
//+------------------------------------------------------------------+
//| MUDANÇAS v5:                                                     |
//| - Sistema de rastreamento completo (TrackingManager)            |
//| - Detecção instantânea sem delays temporais                     |
//| - Comentários únicos com rastreamento de origem                 |
//| - CSV completo com 23 campos de dados                           |
//| - Correção de erros de compilação DEAL_REASON                   |
//| - Correção de conflito de nomes de parâmetros                   |
//+------------------------------------------------------------------+
...
input double   InpATRMultiplier        = 1;       // Multiplicador ATR
...
int OnInit() {
    ...
    // Log de início
    Print("=== EA-HedgeLine v5 Final INICIANDO ===");
    Print("Horário: ", TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS));

    LogToFile("=== EA-HedgeLine v5 Debug INICIANDO ===");
    ...
    Print("  CSV será gravado em: Common/Files/", InpTrackingFile);
    ...
    // Inicializar ReversalManager com TrackingManager ANTES do Panel
    ...
    // Inicializar PanelManager APÓS os módulos que ele vai monitorar
    ...
    // Conectar módulos ao PanelManager IMEDIATAMENTE após Init
    panelMgr.ConnectModules(&trackingMgr, &reversalMgr);
    if(InpDebugMain) Print("★ Módulos TrackingManager e ReversalManager conectados ao PanelManager");

    // Forçar atualização do painel com módulos conectados
    panelMgr.Update();
    if(InpDebugMain) Print("★ Painel atualizado com módulos conectados");
    ...
    // Sistema pronto
    systemReady = true;
    Print("=== HedgeLine v5 Final Iniciado ===");
...
void OnDeinit(const int reason) {
    ...
    Print("★★★ FINALIZANDO TRACKINGMANAGER ★★★");
    trackingMgr.Finalize();
    Print("✓ TrackingManager finalizado - Todos os dados salvos em: ", InpTrackingFile);
...
void OnTick() {
    ...
    if(firstTick) {
        ...
        Print(">>> TrackingManager está ativo e rodando!");
    ...
    // VERIFICAÇÃO PERIÓDICA: Verificar stop losses a cada 10 ticks (backup adicional)
    static int tickCounter = 0;
    tickCounter++;
    if(tickCounter % 10 == 0 && InpUseReverse) {
        static datetime lastPeriodicCheck = 0;
        if(TimeCurrent() - lastPeriodicCheck > 5) { // A cada 5 segundos
...
void OnTrade() {
    ...
    Print("→ [TRACKING] Chamando trackingMgr.UpdateAllStatus()...");
    trackingMgr.UpdateAllStatus();
    Print("✓ [TRACKING] UpdateAllStatus() concluído");
    ...
    if(orderMgr.HasPosition()) {
        ...
        // NOVO: Atualizar lote atual para ReversalManager usar nas reversões
        ...
        Print("→ [TRACKING] Verificando fechamentos recentes via HasRecentClosure()...");
        if(orderMgr.HasRecentClosure(lastProfit, reason, comment)) {
            Print("★★★ [TRACKING] FECHAMENTO DETECTADO VIA TRACKINGMANAGER ★★★");
            ...
            if(reason == DEAL_REASON_SL && InpUseReverse) {
                ...
                Print("  InpUseReverse: ", InpUseReverse);
                Print("  InpMaxReversals: ", InpMaxReversals);

                // Depuração do estado atual do ReversalManager
                ...
                Print("    useReverse: ", currentConfig.useReverse);
                Print("    lastReversalDeal: #", currentState.lastReversalDeal);

                Print("→ [TRACKING/REVERSAL] Chamando ProcessTradeEvent()...");
                bool reversalExecuted = reversalMgr.ProcessTradeEvent();
                Print("← [TRACKING/REVERSAL] ProcessTradeEvent() retornou: ", (reversalExecuted ? "SUCCESS" : "FAILED"));

                if(reversalExecuted) {
                    ReversalState revState = reversalMgr.GetState();
                    stateMgr.UpdateReversals(revState.currentReversals, revState.inReversal);

                    Print("✓ [TRACKING/REVERSAL] REVERSÃO EXECUTADA COM SUCESSO!");
                    Print("  Reversão #", revState.currentReversals, "/", currentConfig.maxReversals);
                    Print("  Estado inReversal: ", revState.inReversal);
                    Print("  Total execuções: ", revState.totalReversalExecutions);
                } else {
                    Print("✖ [TRACKING/REVERSAL] REVERSÃO NÃO EXECUTADA");
                    Print("  Verificar logs detalhados do ReversalManager acima");
                    Print("  Possíveis causas:");
                    Print("    1. Limite de reversões atingido");
                    Print("    2. TrackingManager retornou false");
                    Print("    3. Ticket já foi processado");
                    Print("    4. Erro na execução da ordem");
                }
            }
    ...
    // VERIFICAÇÃO ADICIONAL: Verificar stop losses recentes mesmo quando não há posição detectada
    if(InpUseReverse) {
        if(InpDebugMain) {
            Print("→ [ONTRADE] Verificação adicional de stop losses recentes...");
        }
    ...
    if(InpDebugMain) {
        ...
        double totalProfit;
        int stopLossCount;
        trackingMgr.GetStatistics(totalOrders, openPositions, closedPositions, totalProfit, stopLossCount);
